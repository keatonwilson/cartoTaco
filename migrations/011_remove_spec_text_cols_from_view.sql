-- Migration: Remove redundant specialty text columns from sites_complete view
-- The old string columns (specialty_item_1-3, protein_spec_1-3, salsa_spec_1-2)
-- are now superseded by the embedded specialty_items / specialty_proteins /
-- specialty_salsas JSONB arrays added in migration 010.
-- Underlying table columns are left in place (non-destructive).
--
...
SELECT
  ...
  -- Menu table fields (specialty_item_1-3 text columns removed; use specialty_items array)
  jsonb_build_object(
  ...
  -- Salsa table fields (salsa_spec_1-2 text columns removed; use specialty_salsas array)
  jsonb_build_object(
  ...
  -- Protein table fields (protein_spec_1-3 text columns removed; use specialty_proteins array)
  jsonb_build_object(
...
COMMENT ON VIEW public.sites_complete IS 'Optimized view combining all site-related tables into a single query. Specialty data is embedded as JSONB arrays via FK joins; redundant text columns removed from output.';
