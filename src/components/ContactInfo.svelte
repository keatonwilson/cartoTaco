<script>
  ...
  // Compact mode: an icon-only action row (+ optional address line) used in the
...
{#if hasContactInfo}
  {#if compact}
    <!-- Compact: icon-only action row (+ address line) for the desktop card -->
...
<style>
  ...
  /* ===== Compact mode (desktop card, beside the always-visible hours) ===== */
  .contact-compact {
    display: flex;
    flex-direction: column;
    gap: 6px;
  ...
  :global(.dark) .contact-compact-address {
    color: #d1d5db;
  }

  .contact-compact-address :global(.icon) {
    color: #FE795D;
    flex-shrink: 0;
  }
  ...
  /* Phone / website icon buttons — brand-tinted to match the action row */
  .action-icon {
    display: flex;
    align-items: center;
    justify-content: center;
  ...
  /* Match the social / directions button footprint in the compact row */
  .contact-compact-actions :global(.social-button) {
    width: 36px;
    height: 36px;
  }
</style>
